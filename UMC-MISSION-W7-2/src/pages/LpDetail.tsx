...
const LpDetail = () => {
  ...
  const likeMutation = useMutation({
    mutationFn: async () => {
      ...
      return axios.post(
        `${import.meta.env.VITE_API_BASE_URL}v1/lps/${lp?.id}/likes`,
        {},
        {
          headers: {
    ...
    onMutate: async () => {
      ...
      if (previousData) {
        queryClient.setQueryData(["lpDetail", id], {
          ...previousData,
          likes: [
            ...previousData.likes,
            { id: -1, userId: Number(userId), lpId: previousData.id },
          ],
        });
      }
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(["lpDetail", id], context.previousData); // 롤백 처리
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["lpDetail", id] }); // 서버 데이터 동기화
    },
  });
  ...
  const isLiked = lp.likes.some((like) => like.userId === Number(userId));
