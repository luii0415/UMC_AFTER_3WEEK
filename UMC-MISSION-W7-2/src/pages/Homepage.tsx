...
const Homepage = () => {
  ...
  const throttledIntersecting = useThrottle(isIntersecting, 1000); // 1초 간격으로만 변화 인식
  ...
  useEffect(() => {
    if (throttledIntersecting && hasNextPage) {
      fetchNextPage();
      console.log("추가 get");
    }
  }, [throttledIntersecting, hasNextPage]);
